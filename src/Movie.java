...
public class Movie
{
	...
	{
		q.setClass(Movie.class);
		q.declareParameters("String sName, int n");
		q.setFilter("this.actors.size() >= n && this.studio.name == sName");
		q.setOrdering("releaseYear ascending, title ascending");
		@SuppressWarnings("unchecked")
		Collection<Movie> result = (Collection<Movie>) q.execute(sName, x);
		return result;		
	}
...

	{
		...
		@SuppressWarnings("unchecked")
		Collection<Movie> result = (Collection<Movie>) q.execute(d, y1, y2);
		return result;		
	}
	...
	{
		q.setClass(Movie.class);
		q.setResult("this, this.studio, this.director");
...

	{
		...
		q.setResult("distinct director, count(this)");
		q.setOrdering("count(this) ascending");
		@SuppressWarnings("unchecked")
		Collection<Object[]> result = (Collection<Object[]>) q.execute();
		return result;		
	}
